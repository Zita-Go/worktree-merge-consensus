...
`consensus_apply_patch`。操作者插件的 9 个工具不能作为参与任务可见性的证据。Reviewer
...
0.2.14 只允许在冻结的 Primary worktree 中执行精确命令
`git symbolic-ref --short HEAD`，把它视为只读的当前分支查询。App Server 生成的一层规范
`/bin/bash -lc` 包装会先被去除，再执行同一精确检查；其他 `symbolic-ref` 选项、引用、删除和
写入形式仍全部禁止。显式恢复只会迁移 0.2.13 留下的精确
`BLOCKED / FORBIDDEN_OPERATION` 诊断：
`patch-success confirmation executed a non-read-only command: /bin/bash -lc 'git symbolic-ref --short HEAD'`。
重试前，协调器会重新验证已完成的请求 turn 与绑定、规范只读命令历史、成功受控补丁来源、
未移动的冻结源、干净的授权目标、源祖先关系和权威集成结果。随后只在同一 Run 中原子归档
已完成的确认并重新获取仓库锁，再请求一次结果确认，不会重复补丁、merge、暂存或 commit。
任何近似诊断、带副作用的历史、身份变化或已存在的接受结果都保持终止。

0.2.15 补全了上述迁移在真实生产运行中的恢复形态：唯一成功的受控补丁可以位于已归档的
ephemeral Primary 尝试中，而当前已完成尝试只包含只读结果确认。恢复会分别验证已归档补丁
记录、请求哈希、冻结 Source 谱系和活动绑定，以及当前确认 turn。当前 turn 只允许规范的
用户消息、推理、上下文压缩、最终回复，以及在最终回复之前由 agent 发起、退出码为零、位于
Primary worktree 的精确只读命令；任何 MCP 调用、文件变更、动态工具、写入、不确定命令或
回复后的命令都保持终止。协调器随后重新验证未变化的源和权威目标，只归档当前确认，并在
同一 Run 上重试，不改变集成分支、commit 或唯一补丁记录。因此，如果 0.2.14 的显式恢复在
修改状态之前以 `MODEL_RESPONSE_RETRY_UNSAFE` 失败，而原始精确 0.2.13 blocker 仍被记录，
可以安装匹配的 0.2.15 二进制与插件后再次显式恢复。

0.3.0 会把共识过程展示在启动它的 Codex 任务中。每次持久 Run 状态变化都会在同一个
SQLite 事务中写入一条有界公开事件；启动任务通过可恢复游标持续显示阶段、轮次、双方契约、
主修方案、复核意见、集成分支与 SHA、冻结测试证据以及最终验收。`after_cursor` 可在观察中断后
从已有游标继续。无变化的长轮询心跳保持很小，
公开流不会包含隐藏推理、参与任务提示词、原始任务历史或命令 stdout/stderr。

0.3.1 在冻结的 Primary worktree 中接受精确只读查询
`git branch --show-current`，可直接执行，也可带一层规范的 `/bin/bash -lc` 包装。协调器本来就会
在 Primary 集成前后自行推导并校验当前分支与 HEAD；提示词现在明确了这一职责，并列出两种受支持
的等价查询，因此模型选择哪条等价命令不再影响结果。旧版本仅因该查询而阻塞的无副作用 Run 可在
显式恢复后继续，包括补丁成功后的已完成确认：恢复必须重新验证规范终态历史、成功补丁记录、未移动
的冻结引用和权威目标，而且绝不会重复补丁、merge、暂存或 commit。公开字段
`no_progress_rounds` 表示连续不变审查的配置阈值，不是当前计数；方案内容发生进展会改变审查
fingerprint，并开始新的 streak。
...
任务中调用 `$worktree-merge-consensus`；插件提供 9 个 MCP 工具。其中 8 个（包括
`consensus_list_worktrees` 与 `consensus_wait`）用于启动、观察和控制持久协调器；
`consensus_apply_patch` 是下文说明的、仅供内部参与 turn 使用且绑定精确请求的写入能力。
它不会引入第三个 agent 代为转发复核对话。

操作者插件的 9 个工具不是主修参与任务的工具清单。协调器会通过直接或 ephemeral
...
持续查看公开事件流，直到 Run 完成或暂停。观察中断后可用 `--after-cursor` 从已有游标继续；
`--json` 会按 JSON Lines 每行输出一个事件批次。
...
9 个公开命令组是 `codex-consensus configure`、`codex-consensus doctor`、
`codex-consensus threads`、`codex-consensus worktrees`、`codex-consensus run`、
`codex-consensus status`、`codex-consensus watch`、`codex-consensus resume` 和
`codex-consensus cancel`。需要自动化时，可在 `--help` 标明的操作叶节点使用稳定 JSON 输出；
`watch --json` 在持续观察期间使用 JSON Lines。
...
- 如果 v0.3.0 或更早版本的 `FORBIDDEN_OPERATION` 仅由冻结 Primary worktree 中的精确
  `git branch --show-current` 导致，安装版本匹配的 v0.3.1 二进制与插件后，可对同一 Run
  调用一次 `consensus_resume`。任何不确定命令、副作用、身份不匹配或仓库状态变化仍会拒绝恢复。

...
源仓库写入根目录；单独的验证 turn 只能写入隔离克隆。协调器启动的所有 turn 都显式使用
App Server `approvalPolicy = never`，因此两个参与任务不会再为命令或文件操作等待人工批准。
只有狭窄 Git 白名单内的集成命令和每条恰好执行一次的冻结测试才能被验收。每个命令都必须在
预期 cwd 中恰好出现一次，并对应退出码
为 0 的 App Server `commandExecution` 项；模型自行声称“通过”不构成证据。冻结测试必须是
直接、非 Git 命令；Git 可执行文件、shell 控制符以及动态 shell/解释器启动器都会被拒绝。
若事件证据包含发布、破坏性 Git、shell 串联、错误目录命令或权限升级，协调器会拒绝验收。
断网且仅开放 Run 范围写入根目录的 sandbox 仍是预防边界；由于 `never` 刻意取消了执行前的
人工复核，无人值守流程只应运行可信任务与可信仓库内容。冲突扫描依据

...
fn action_instruction(action: NextAction) -> &'static str {
    match action {
        ...
        NextAction::RequestPrimaryVerification => {
            "This is a test-only turn in an isolated local clone that cannot write the source repository's Git common directory. Invoke the command-execution tool once for each required_test_commands entry, in the listed order, using each exact command as one standalone tool call. Always invoke the first command instead of inferring that its executable is unavailable. Continue through every remaining entry even when an earlier command exits nonzero, and wait for every command call to reach a terminal state. Run every entry exactly once and run no other command. Do not edit files or invoke Git. After all required commandExecution items exist, return the VERIFICATION_READY marker regardless of individual exit codes; VERIFICATION_READY means the evidence set is complete, not that every test passed. A final answer without all command items is invalid evidence. The coordinator derives pass/fail results and diagnostic output from those commandExecution items and routes failures back to the integration turn."
        }
        NextAction::RequestReviewerResultVerdict => {

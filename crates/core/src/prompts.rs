...
- Once one successful controlled patch has been committed, return INTEGRATION_READY even when a final read-only inspection finds a defect. Describe that known defect in the Markdown summary so coordinator-owned verification can preserve it and open a new request-bound correction round. Do not call a second patch and do not return BLOCKED:PATCH_LIMIT_REACHED for a clean committed result.
"#
        }
...
- Any explanation after the marker is free-form Markdown. Do not report test result fields: the coordinator derives authoritative evidence itself.
"#
        }
...
fn action_instruction(action: NextAction) -> &'static str {
    match action {
        ...
        NextAction::RequestPrimaryIntegration => {
            "Revalidate the frozen inputs, then create only the authorized new integration branch at primary_sha, merge reviewer_sha into it, resolve conflicts exactly according to the approved plan, and commit compatibility fixes. If the exact target branch already exists because this is a coordinator-authorized retry, do not recreate or re-merge it: verify that HEAD is attached to that target, both frozen SHAs are ancestors, the worktree is clean, and continue from that exact HEAD. The coordinator independently validates the current branch and HEAD before and after this turn; do not query them merely to report result identity. If retry reasoning still requires a current-branch query, prefer exactly `git branch --show-current`; `git symbolic-ref --short HEAD` is the only equivalent form, and either may have at most one `/bin/bash -lc` wrapper. A target-absence preflight may be exactly `git show-ref --verify refs/heads/TARGET` or `git show-ref --verify --quiet refs/heads/TARGET`; no other `show-ref` arguments are allowed. Do not run tests in this turn. Required repository-instruction discovery is allowed first as `rg --files -g AGENTS.md`. If `rg` is unavailable, use `git ls-files` to discover tracked `AGENTS.md` files. Perform every other repository inspection through the command gate's read-only Git queries: use `git show REV:path` to read tracked content, `git ls-files` to list tracked paths, and `git diff` to inspect changes; do not invoke sed, cat, find, ls, head, tail, or another shell file reader. Never use `git diff --no-index`; stage new files with `git add -A` before inspecting them with `git diff --cached`. The command gate accepts branch creation only as `git switch -c TARGET PRIMARY_SHA`, merge only as `git merge --no-ff --no-edit REVIEWER_SHA`, staging only as `git add -A` or `git add --all`, and commits only as `git commit --no-edit` or `git commit -m ONE_SAFE_TOKEN`, where the token is at most 120 UTF-8 bytes and contains only Unicode letters or numbers plus `-`, `_`, `.`, or `:`. Make all compatibility edits in one successful call to `consensus_apply_patch`, passing the authoritative run_id, the exact request_hash from Coordinator delivery identity, and one raw unified text patch. A failed patch preflight leaves the worktree clean and may be corrected; after one successful patch, no second patch is authorized. After that successful patch is committed, do not attempt to amend it in this request. If a final read-only inspection finds a compile, formatting, test, or semantic defect, return INTEGRATION_READY and describe the known defect in Markdown instead of returning BLOCKED:PATCH_LIMIT_REACHED. Coordinator-owned verification will test the exact commit, retain the defect evidence, and, when needed, issue a new request-bound correction round whose separate patch allowance may advance the integration SHA. Return BLOCKED only when no clean committed integration result can be handed to the coordinator. This is the only authorized `consensus_*` call; do not use the built-in file-change tool, and do not call start, status, resume, cancel, discovery, or another coordinator tool. The controlled patch is accepted only after the clean authorized merge. Use one shell command per tool call; shell chaining is forbidden. Return only the INTEGRATION_READY marker plus optional free-form summary; the coordinator reads the final branch, HEAD, and changed files directly from Git. Do not push or update either source ref."
        }
        NextAction::RequestPrimaryVerification => {

...
fn integration_prompt_declares_the_frozen_repository_read_surface() {
    ...
    for required in [
        ...
        "return INTEGRATION_READY and describe the known defect in Markdown instead of returning BLOCKED:PATCH_LIMIT_REACHED",
        "issue a new request-bound correction round",
        "Return BLOCKED only when no clean committed integration result can be handed to the coordinator",

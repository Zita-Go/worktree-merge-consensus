...
| `VERIFY` | The coordinator creates a detached, remote-free clone of the exact result SHA. A separate primary turn runs each frozen test there exactly once, continuing after failures; the daemon derives exit codes and bounded diagnostics from completed App Server command items and confirms both source refs are unchanged. A failed command returns the same Run to another controlled integration round. |
